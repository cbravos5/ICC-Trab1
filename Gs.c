...
void Gs(struct matPent m, int afast, int MAX, int ite){
	...
	while(j < ite){
		...
		//PRIMEIRA ATRIBUICAO
		m.X[0] = (m.B[0] - m.Ds[0]*Xold[1] - m.Dsa[0]*Xold[afast + 2])/m.Dp[0]; 

		//LOOP EM QUE DIAGONAL INFERIOR AFASTADA AINDA NAO APARECEU
		for(int i = 1; i < afast + 2; i++)
			m.X[i] = (m.B[i] - m.Di[i]*m.X[i - 1] - m.Ds[i]*Xold[i + 1] - m.Dsa[i]*Xold[afast + 2 + i])/m.Dp[i];

		//LOOP EM QUE AMBAS DIAGONAIS AFASTADAS ESTAO PRESENTES
		for(int i = afast + 2; i < MAX - afast - 2; i++)
			m.X[i] = (m.B[i] - m.Dia[i]*m.X[i - afast - 2] - m.Di[i]*m.X[i - 1] - m.Ds[i]*Xold[i + 1] - m.Dsa[i]*Xold[afast + 2 + i])/m.Dp[i];
		
		//LOOP EM QUE DIAGONAL SUPERIROR AFASTADA ACABOU
		for(int i = MAX - afast - 2; i < MAX - 1; i++)
			m.X[i] = (m.B[i] - m.Dia[i]*m.X[i - afast - 2] - m.Di[i]*m.X[i - 1] - m.Ds[i]*Xold[i + 1])/m.Dp[i];
		
		//ULTIMA ATRIBUICAO
		m.X[MAX - 1] = (m.B[MAX - 1] - m.Dia[MAX - 1]*m.X[MAX - afast - 3] - m.Di[MAX - 1]*m.X[MAX - 2])/m.Dp[MAX - 1];
